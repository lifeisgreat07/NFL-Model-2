...
| [2026-09-21](2026-09-21.md) | Queue item 1 recorded and closed on `main`, and #79 opened for the `<noscript>` note that fell out of it. Both premises re-executed first and both held: the `#8A93A8` chevron is dead rather than wrong in light mode, and the no-script reader those two selects were kept for cannot reach either page. The correction's real blast radius was eight sites against the three proposed, found only because the scope came from a repo-wide search; one of them was a neighbouring component's docstring repeating the same false sentence. Two lessons on tools: `runner.py` honours only the last `--id` and still prints "all caught", and `check_visual_claims_have_artifacts` prescribes a process-note remedy its code will reject |
| [2026-09-22](2026-09-22.md) | #82 and #83 merged. #83's new step caught a real silent Booth audit on its first live run, #84, inside the duration range the old tell called normal. Stage 5 was registered before it was answered and accepted nothing. Its one signal is that the live pipeline rates last game's quarterback while the published backtest rates the game's own, a gap nothing had compared before. Later: #84–#86 merged (v2.5 rates the expected starter, with sourced overrides), #87 merged Stage 9, and the Tuesday lock time surfaced as the reason late injury news cannot reach the picks |
| [2026-09-23](2026-09-23.md) | #88 opened and audited SAFE TO MERGE: picks lock on Thursday before Thursday night's kickoff instead of Tuesday morning, so late QB news can reach them. The rule locks when the first game comes before the next scheduled run, which handled 2026's two Wednesday openers, which nobody had planned for, without a special case |
| [2026-09-24](2026-09-24.md) | #96 and #97 merged, #98 opened: Stage 7's case studies, lessons, architecture diagram and README, plus the About panel. The Booth regression suite ran for the first time after seventeen days and caught its planted defect. A leak table split two ways across identical Linux environments, and my first explanation of it was wrong |
| [2026-09-24-2](2026-09-24-2.md) | Stage 4 finished in one evening: eight PRs, #100 to #107, each merged only after Booth said SAFE. The reproducibility audit reproduced every published backtest figure exactly, on Windows and again on Booth's Linux runner. Booth caught one PR body quoting a shorter command than the one run, which hid a crash; a drift guard's mutation survived once a later step was added inside the span it sliced |
