...
- **Tests that are themselves tested.** A committed mutation corpus
  (`tests/mutation/`, 65 case files, one per subject under test)
...
- **Ten CI workflows** in `.github/workflows/` cover the test suite, the
  ...
  fails, and a nightly canary that runs the weekly data path and raises an
