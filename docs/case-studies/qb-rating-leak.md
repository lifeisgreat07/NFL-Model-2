...
**How far these figures travel.** This
table came from Windows, Python 3.11.9, with the library versions recorded in
the JSON's `provenance`. Booth then re-ran the script on Linux in each of its
[four audits of #96](https://github.com/lifeisgreat07/NFL-Model-2/pull/96).
The second and third matched this table in every figure it shows. The first
and fourth agreed with each other and not with it: one fewer pick changing
side for each model, and different last digits in every score. Audits on both
sides reported the same library versions as this file, so the versions do not
explain the split, and what does is not known. The verdict held in every run.
Read the pick counts and last digits as this run's, and the verdict as the
result.
